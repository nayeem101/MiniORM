...
public class UnitOfWork : IUnitOfWork
{
    ...
    public async Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var entry in changedEntries)
        {
            ...
            {
                case EntityState.Added:
                    affectedRows += await InsertEntityAsync(entry, cancellationToken);
                    break;

                case EntityState.Modified:
                    affectedRows += await UpdateEntityAsync(entry, cancellationToken);
                    break;

                case EntityState.Deleted:
                    affectedRows += await DeleteEntityAsync(entry, cancellationToken);
                    break;
            }
        }
    ...
    private async Task<int> InsertEntityAsync(EntityEntry entry, CancellationToken cancellationToken)
    {
        var metadata = EntityMapper.GetMetadata(entry.EntityType);
        var insertQuery = new InsertQueryBuilder()
            .Into(metadata.TableName);
        ...
        var sql = insertQuery.Build();
        var id = await _context.ExecuteScalarAsync(sql, insertQuery.GetParameterTuples());
    ...
    private async Task<int> UpdateEntityAsync(EntityEntry entry, CancellationToken cancellationToken)
    {
        var metadata = EntityMapper.GetMetadata(entry.EntityType);
        var updateQuery = new UpdateQueryBuilder()
            .Table(metadata.TableName);
        ...
        var sql = updateQuery.Build();
        return await _context.ExecuteNonQueryAsync(sql, updateQuery.GetParameterTuples());
    }

    private async Task<int> DeleteEntityAsync(EntityEntry entry, CancellationToken cancellationToken)
    {
        var metadata = EntityMapper.GetMetadata(entry.EntityType);
        var deleteQuery = new DeleteQueryBuilder()
            .From(metadata.TableName);
        ...
        var sql = deleteQuery.Build();
        return await _context.ExecuteNonQueryAsync(sql, deleteQuery.GetParameterTuples());
    }
